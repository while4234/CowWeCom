...
| `cowwechat-project-optimizer` | Admin-only local optimizer for CowWeCom cache-hit analysis, reasoning-effort tuning, Agent tool-chain review, temporary-script preservation, and Codex daily 300-call gated optimization using ignored runtime evidence. | No external API key; raw evidence stays local and is consumed/deleted by the optimizer report. |
...
| `grok-image-prompt-optimization` | Grok-only image prompt polishing, random prompt generation, fuzzy fragment matching, and NSFW/大尺度 Grok fragment selection. | Local Grok/xAI OAuth login for Grok prompt rewriting and translation. |
| `image-prompt-optimization` | Hidden prompt reference library for non-Grok GPT/Codex image generation. | No external API key for repository lookup. |
