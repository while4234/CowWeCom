...
知识库写入按来源分层：用户分享的文章/链接/文档、联网搜索并核验到的公开事实、公开实体和公开资料结论，可以按 `knowledge-wiki` 自动沉淀，方便下次复用。

协议/规范/状态机/表格/encoding/跨章节问题中，由 AI 基于证据包分析出的结论默认只读；除非用户明确说“保存到个人知识库/记入知识库/整理沉淀”，不要自动写入 `knowledge/`。

如果证据不足、missing terms 未覆盖关键点，或回答包含“可能/推测/证据不足”，不得写入知识库，只提示继续核验。

**边界**：普通微信用户、私人称呼、家庭/亲密关系、吵架矛盾、桥接可达状态、Bridge ID、微信渠道、跨用户转述内容不是知识库内容。
这些只能写入用户记忆或关系记忆，除非用户明确要求整理成知识库文档。
...
如果用户明确说“我叫 X / 叫我 X / 以后叫我 X / 你叫 Y”，这不是闲聊：
- 立即更新 `USER.md`。
- `X` 应替换为唯一的“当前称呼”；若原来已有不同当前称呼，把旧值移入“别称/曾用称呼”，不要覆盖丢失。
- `Y` 应更新为助手名称或助手别称。
- 第一次说的名字和后来新说的名字都要保留；平时主动称呼用户只使用最新明确的“当前称呼”，旧称呼只用于识别同一用户。
