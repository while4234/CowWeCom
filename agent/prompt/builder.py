...
def _build_skills_section(skill_manager: Any, tools: Optional[List[Any]], language: str) -> List[str]:
    ...
    lines = [
        ...
        "当有匹配的技能时，应优先使用技能",
        "- 如果多个技能互斥或高度重叠，选择最匹配的一个。",
        "- 如果多个技能能互补完成同一任务，应读取必要的多个 SKILL.md 并按职责配合使用；保持集合最小，通常不超过 3 个。",
        "- 例如：旅行/一日游规划同时涉及路线、ETA、路况或交通方式判断时，应组合旅行规划类 Skill 与地图/高德路线类 Skill。",
        "- 如果没有技能明确适用：不要读取任何 SKILL.md，直接使用通用工具。",
        "",
        f"**重要**: 技能不是工具，不能直接调用。使用技能的唯一方式是用 `{read_tool_name}` 读取 SKILL.md 文件，然后按文件内容操作。"
        "不要读取与当前任务无关的技能；已读取一个技能后，如果另一个技能明确互补，也可以继续读取。",
        "",
        "以下是可用技能："
    ]
...
def _build_knowledge_section(workspace_dir: str, language: str) -> List[str]:
    ...
    lines = [
        ...
        "### 自动写入规则（mandatory）",
        "",
        "知识库写入按来源分层：公开来源可以自动沉淀，协议/规范类 AI 推导默认只读。",
        "不要把协议/规范/状态机/表格/encoding/跨章节问题中由 AI 分析出的结论自动写入 `knowledge/`；只有用户明确说“保存到个人知识库/记入知识库/整理沉淀”才写入。",
        "如果证据不足、missing terms 未覆盖关键点，或回答包含“可能/推测/证据不足”，不得写入知识库，只提示继续核验。",
        "",
        "以下场景可以在回复的同时写入知识库：",
        "",
        "1. **用户分享了文章/链接/文档** → 阅读理解后，在同一轮回复中将要点写入 `knowledge/sources/<slug>.md`",
        "2. **联网搜索并核验到公开事实/公开实体/公开资料结论** → 可按 `knowledge-wiki` 写入，方便下次复用",
        "3. **用户明确要求保存/整理沉淀** → 可写入，并标注来源为当前回答和原文证据",
        "",
        "### 知识库边界",
        "",
        ...
        "",
        "协议/规范深问的回答末尾应自然提醒：这是基于当前证据的 AI 总结，用户确认可靠后可回复“保存到个人知识库”。",
        "",
    ]
...
def _build_workspace_section(workspace_dir: str, language: str) -> List[str]:
    ...
    lines = [
        ...
        "- ✅ `USER.md`: 已加载 - 用户的身份信息。当用户修改称呼、姓名等身份信息时，用 `edit` 更新此文件；如果出现新称呼，把它设为唯一当前称呼，旧称呼只保留为别称/曾用称呼",
