...
When adding or fixing a builtin skill in the project repository, update both
`<project_root>/skills/<skill>/` and the deployed `~/cow/skills/<skill>/` copy
when that workspace copy exists. Workspace skills have higher priority and
override builtin skills at runtime, so changing only one copy can make local
behavior and GitHub deployments diverge. Publish fixes by committing and
pushing the project copy, and never copy secrets, caches, snapshots,
`__pycache__`, or other local runtime state into a skill directory.
