...
Start the program and open the Web console (local access: http://127.0.0.1:9899). Go to the **Channels** tab, click **Connect Channel**, select **WeCom Bot**, scan to create a bot or fill in the Bot ID and Secret from the previous step, and click Connect. The QR flow uses `wecom_bot_auth_source`; if QR-code creation reports invalid parameters, confirm the source value or use the manual flow.
