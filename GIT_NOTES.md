...
- Latest local code work: social bridge tone and private knowledge boundary fix
...
- `2026-05-22` `fix: soften bridge tone and protect private knowledge`: Made social bridge generated messages sound more like a warm friend relaying a message, removed stiff public wording such as `授权我转述`, added a non-apology bridge-message test, added prompt/skill/tool guardrails that keep private Weixin users, Bridge IDs, social-bridge reachability, and cross-user relay metadata out of `knowledge/`, and cleaned the local workspace knowledge page that had captured 小栀/Rondle bridge details. Validation: `.venv312\Scripts\python.exe -m unittest tests.test_knowledge_guard tests.test_social_bridge_service tests.test_social_bridge_store tests.test_social_bridge_tools tests.test_multi_weixin_instances tests.test_multi_user_isolation tests.test_fast_lane_progress` OK (65 tests); `.venv312\Scripts\python.exe -m compileall agent/prompt/builder.py agent/prompt/workspace.py agent/social_bridge/service.py agent/tools/knowledge_guard.py agent/tools/write/write.py agent/tools/edit/edit.py tests/test_knowledge_guard.py tests/test_social_bridge_service.py` OK; `git diff --check` OK with CRLF warnings only.
