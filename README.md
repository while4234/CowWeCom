...
| 日期 | 更新 |
| --- | --- |
| 2026-05-26 | 合并 README 当天更新记录并固化“同一天只保留一条总结”的项目规则；同时完成聊天内 CowCli 权限分级与角色化 `/help`、本地账本按当前 `memory_user_id` 查询、`china-expense-ledger` 本地记账/截图澄清/汇总缓存优化、`work-progress-reporter`、`project-restart`、`cowagent-workflow-auditor` 与 `cowwechat-project-optimizer` Skills、本地 `.venv` 验证环境统一、项目优化每日 0 点按 300 次增量模型调用触发，以及 LLM 后端额度状态和 CAPI 月卡日重置展示修复 |
