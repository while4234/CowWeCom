...
- Grok real-mode 的 `expression` 表情字段改为默认不插入表情，只有显式选择 `随机`、填写素材 key 或 `custom:` 时才会写入最终 prompt。
- Grok real-mode 的 `color_tone` 色调字段也改为默认不插入色调，只有显式选择 `随机`、填写素材 key 或 `custom:` 时才会写入两个色调位置。
