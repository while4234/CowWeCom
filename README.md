...
- 新增管理员专用 `/grok-direct image|video` 直出模式，原始 prompt 直接提交到 Grok 生图/视频后台任务，不经过 Agent 提示词分析、检测或润色；默认图片 speed、视频 `480p / 16:9 / 6s`，图片直出会跳过 YouMind 隐式提示词增强。
- `/grok-direct video` 和普通 Grok 视频会把可解析的消息图片或同会话最近图片作为参考图，支持“参考上图/上面几张”图生视频；企业微信引用旧图若回调只给纯文本，机器人无法定位被引用图片。
