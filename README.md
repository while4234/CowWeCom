...
- 普通 Grok 生图/视频润色默认 90% 使用 `grok` 仓库、10% 使用其他仓库；prompt 含 `NSFW` 时强制路由到 `grok/NSFW` 分类。
