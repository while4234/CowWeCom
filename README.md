...
| 2026-05-26 | 优化聊天内 CowCli 权限和帮助排版：`/help` 会按管理员/普通用户过滤可用命令；普通用户可用状态、版本、账本、Skill 查询、知识库查询、后端状态和额度查询；后端切换、配置修改、日志、浏览器依赖安装、Skill 安装/卸载/启停、知识库开关、记忆蒸馏/重建索引等高危动作需要管理员权限。新增 CLI 权限表，未分类的新命令默认管理员可用 |
| 2026-05-26 | 修复本地账本查询走错用户的问题：聊天中“查询本日/本周/本月/上月账单”会由 CowCli 直接使用当前微信/企业微信上下文的 `memory_user_id` 查询本地 SQLite，不再走 Skill 示例里的 `local-user` 占位；企业微信群中也会按实际发言人判断管理员权限 |
